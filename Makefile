...
docker-build:  ## Builds normal docker container
	docker build -t mark1ns0n/rtsp-stream:${DOCKER_VERSION} .
docker-debug: ## Builds the image and starts it in debug mode
	...
	-p 8080:8080 \
	mark1ns0n/rtsp-stream:${DOCKER_VERSION}
docker-build-mg:  ## Builds docker container with management UI
	docker build -t mark1ns0n/rtsp-stream:${DOCKER_VERSION}-management -f Dockerfile.management .
docker-debug-mg: ## Builds management image and starts it in debug mode
	...
	mark1ns0n/rtsp-stream:${DOCKER_VERSION}-management
